package com.epam.javalab.codingbat.logic_2;
...
public class Solution {
    ...
    public boolean makeBricks(int small, int big, int goal) {
        if (goal - big * 5 <= 0 && goal % 5 <= small) return true;
        if (goal - big * 5 > 0 && goal - big * 5 <= small) return true;
        return false;
    }
     ...
     * Return true if the three values are evenly spaced, so the difference between small and medium is the
    ...
    public int makeChocolate(int small, int big, int goal) {
        if (goal - big * 5 <= 0 && goal % 5 <= small) return goal%5;
        if (goal - big * 5 > 0 && goal - big * 5 <= small) return goal - big * 5;
        return -1;
    }

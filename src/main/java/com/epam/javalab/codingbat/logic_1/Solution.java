package com.epam.javalab.codingbat.logic_1;
...
public class Solution {
    ...
    public int caughtSpeeding(int speed, boolean isBirthday) {
        if (isBirthday && speed <= 65 || !isBirthday && speed <= 60) return 0;
        if (isBirthday && speed <= 85 || !isBirthday && speed <= 80) return 1;
        return 2;
    }

    /**
     * Given 2 ints, a and b, return their sum. However, sums in the range 10..19 inclusive, are forbidden, so
    ...
    public String alarmClock(int day, boolean vacation) {
        if (day > 0 && day < 6) return !vacation ? "7:00" : "10:00";
     ...
     * by 5 use "Buzz", and if divisible by both 3 and 5, use "FizzBuzz". Note: the % "mod" operator computes
     * the remainder after division, so 23 % 10 yields 3. What will the remainder be when one number divides evenly
     ...
     * a non-negative int n is to convert it to a string with String.valueOf(n) and then check the length of the
     * string.)
